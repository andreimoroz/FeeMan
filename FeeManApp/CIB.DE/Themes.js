...
CIB.DE.themes = function () {

    if (!CIB.DE.utilities)
        throw new Error("Utilities.js must be referenced to use Themes");

//    var ctx = CIB.DE.utilities.getContext();

    var themeLoading = false;
    var themeCached = new jQuery.Deferred();
    var themeCache = new CIB.DE.utilities.CIBcache('CIB.DE.App.NavigationLinks');

    $("<link />", {
        rel: "stylesheet",
        type: "text/css",
        href: $.getServerRelativeHostWebUrl().replace(new RegExp("[\/]+$"), "") + "/_layouts/15/defaultcss.ashx"
    }).appendTo("head");
    ...
    var failHandler = function (message) {
        ...
        if (CIB.DE.logging) CIB.DE.logging.logError('Themes Helper', message, window.location.href);
        themeLoading = false;
    };
