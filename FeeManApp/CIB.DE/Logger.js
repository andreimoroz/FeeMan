...
CIB.DE.logging = function () {
    ...
    return {
        ...
        logError: function (category, message, stacktrace) {
            ...
            if (!CIB.DE.logging.applicationName) {
                if (typeof _spPageContextInfo !== 'undefined' && _spPageContextInfo && _spPageContextInfo.webTitle) {
                    CIB.DE.logging.applicationName = _spPageContextInfo.webTitle;
                }
                else {
                    CIB.DE.logging.applicationName = $.getServerRelativeHostWebUrl();
                }
            }

            if (console) {
                console.log('Logging error for: ' + CIB.DE.logging.applicationName);
            ...
            $.ajax({
                ...
                url: loggingService,
                data: JSON.stringify({
                    Application: CIB.DE.logging.applicationName,
                    Category: category,
                    Message: message,
                    StackTrace: stacktrace ? stacktrace : window.location.href
                })
            })
            .fail(CIB.DE.logging.logErrorFailed);
        },
        ...
        logEventAsyncPromise: function (application, eventType, xmlData) {
            ...
            .fail(function (sender, status) {
                CIB.DE.logging.logErrorFailed(sender, status);
